...
def gen_auto_mask_table(topo_file_path, npes, reentrant_x, reentrant_y, tripolar_n, output_dir):
    ...
    if num_masked_blocks == 0:
        raise RuntimeError("Couldn't auto-eliminate any land blocks. Try to increase the number")
...
def write_auto_mask_file(mask_table, idiv, jdiv, npes, output_dir, filename='MOM_auto_mask_table'):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', metavar='npes', type=int, required=True,
                        help='Number of MOM6 PEs (NTASKS_OCN)')
    parser.add_argument('-rx', default=False, action='store_true',
                        help='Is the domain reentrant in x-dir?')
    parser.add_argument('-ry', default=False, action='store_true',
                        help='Is the domain reentrant in y-dir?')
    parser.add_argument('-tn', default=False, action='store_true',
                        help='Is the domain tripolar?')
    parser.add_argument('-o', metavar='output_dir', type=str, required=False,
